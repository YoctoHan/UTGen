...
这是一个基于大语言模型的算子单元测试自动生成工具，支持华为CANN算子的测试代码生成。工具采用两阶段设计，可以灵活地生成测试参数和测试代码。
...
├── prompt_generator.py # Stage 2: Prompt生成器
├── model_caller.py     # Stage 2: 模型调用器
├── post_processor.py   # Stage 2: 后处理器
├── utils.py           # 通用工具函数
│
├── ut-template/       # 单测模板目录
│   └── ut_template.cpp
...
1. **收集Few-shot示例**：从`tiling-examples`目录收集相似算子的测试代码和参数
2. **分析目标算子源码**：解析目标算子的接口和参数
3. **生成测试参数**：通过AI模型生成CSV格式的测试参数组合
...
输出：
- `test_params_matmulallreduce.csv` - 测试参数文件
- `prompt_testcase_matmulallreduce.txt` - 生成时使用的prompt

### Stage 2: 单测代码生成

1. **生成Prompt**：结合算子源码、UT模板和测试参数
2. **调用模型**：通过AI模型生成完整的单测代码
3. **后处理**：提取、清理和格式化生成的代码
...
- `prompt_matmulallreduce.txt` - 生成时使用的prompt
- `generation.log` - 生成日志
...
├── test_params_allgathermatmul.csv    # 测试参数（Stage 1）
├── prompt_allgathermatmul.txt         # 使用的prompt
